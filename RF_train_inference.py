...
#RW_comb['Elev-Terrain height']=RW_comb['Elevation (m)']-RW_comb['Terrain_height(m)']
...
for j in range(int(r)):
    ...
    #FIXME n_jobs
    RF = RandomForestRegressor(random_state=10,n_jobs=-1,**Tuned_HPs)
    model=RF.fit(X_train_copy, Y_train)
    ...
    def Heat_bin_plots(MINXY,MAXXY,INCR,Y_pred,Y_test,c_min,c_max,
        ...
        if title_log==1:
            #FIXME title 
            ax.set_title("Predicted wind gust(RF_tuned) vs. observed wind gust",fontsize = titlesize)
        if xlabel_log==1:
    ...
    #FIXME
    plt.savefig(cd+'/Updated_Stations_RF_TH_as_feature/RF_tuned_'+str(j)+'.png',dpi=300,bbox_inches='tight')
    plt.close()
    ...
    #FIXME
    plt.savefig(cd+'/Updated_Stations_RF_TH_as_feature/RF_tuned_FI'+str(j)+'.png',
    ...
    #FIXME
    plt.savefig(cd+'/Updated_Stations_RF_TH_as_feature/UPP_vs_Obs_test_set_'+str(j)+'.png',
                dpi=300,bbox_inches='tight')
    plt.close()
    ...
    Y_train.to_csv(cd+'/Updated_Stations_RF_TH_as_feature/Y_train_'+str(j)+'.csv')
    #X_test_copy.to_csv(cd+'/RF_61_diff/X_test_'+str(j)+'.csv')
    Y_test.to_csv(cd+'/Updated_Stations_RF_TH_as_feature/Y_test_'+str(j)+'.csv')
    Y_pred.to_csv(cd+'/Updated_Stations_RF_TH_as_feature/Y_pred_'+str(j)+'.csv')
    Y_pred_train.to_csv(cd+'/Updated_Stations_RF_TH_as_feature/Y_pred_train_'+str(j)+'.csv')
    UPP_test.to_csv(cd+'/Updated_Stations_RF_TH_as_feature/UPP_test_'+str(j)+'.csv')
...
all_score.to_csv(cd+'/Updated_Stations_RF_TH_as_feature/Scores_FI_RF_tuned_61_storms.csv', index = False)

Y_test_all.to_csv(cd+'/Updated_Stations_RF_TH_as_feature/All_obs_test_RF.csv', index = False)
Y_pred_test_all.to_csv(cd+'/Updated_Stations_RF_TH_as_feature/All_pred_test_RF.csv', index = False)
#Y_pred_train_all.to_csv(cd+'/Stations_removed_RF_TH_as_feature/All_pred_train_RF.csv', index = False)
UPP_test_all.to_csv(cd+'/Updated_Stations_RF_TH_as_feature/All_UPP_test_RF.csv', index = False)

Y_train_all.to_csv(cd+'/Updated_Stations_RF_TH_as_feature/All_obs_train_RF.csv', index = False)
Y_pred_train_all.to_csv(cd+'/Updated_Stations_RF_TH_as_feature/All_pred_train_RF.csv', index = False)
...
# output file name
#FIXME filename
plot_file_name="FI_RF_Tuned_61_storms.jpg"
 
# save as jpeg
Tplot.figure.savefig(cd+'/Updated_Stations_RF_TH_as_feature/'+plot_file_name,bbox_inches='tight',
                    format='jpeg',
                    dpi=300)
